                    ...
                    <div class="sidebar-scrollable">
                        ...
                        <div class="sidebar-section mb-5">
                            ...
                            <ul class="category-list">
                                ...
                                <li>
                                    <a href="{{ route('shop', ['category' => is_object($category) ? $category->id : $category['id']]) }}"
                                        class="{{ request('category') == (is_object($category) ? $category->id : $category['id']) ? 'active' : '' }}">
                                        {{ is_object($category) ? $category->name : $category['name'] }} ({{
                                        is_object($category) ? $category->products_count : $category['products_count']
                                        }})
                                    </a>
                                </li>
                                @endforeach
                                ...
                                <div class="price-range-inputs">
                                    <div class="mb-3">
                                        <div class="price-range-label">
                                            ...
                                            <span class="price-range-info">$<span id="min-price-display">{{ $minPrice
                                                    }}</span></span>
                                        </div>
                                    ...
                                    <div class="mb-3">
                                        <div class="price-range-label">
                                            ...
                                            <span class="price-range-info">$<span id="max-price-display">{{ $maxPrice
                                                    }}</span></span>
                                        </div>
                                        <div class="price-input-group">
                                            <span class="price-symbol">$</span>
                                    ...
                                    </div> <button type="submit" class="filter-btn"><i
                                            class="fas fa-filter mr-2"></i>Apply Filter</button>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <div class="showing-results">
                                    Showing <span class="badge">{{ $products->firstItem() ?? 0 }}-{{
                                        $products->lastItem() ?? 0 }}</span> of <span class="badge">{{
                                            ...
                                            <div class="select-container">
                                                <select name="sort" id="sort-select" onchange="this.form.submit()">
                                                    ...
                                                    <option value="popular" {{ request('sort')=='popular' ? 'selected'
                                                        : '' }}>Popularity</option>
                                                    <option value="price_low" {{ request('sort')=='price_low'
                                                        ? 'selected' : '' }}>Price: Low to High</option>
                                                    <option value="price_high" {{ request('sort')=='price_high'
                                                        ? 'selected' : '' }}>Price: High to Low</option>
                                                    <option value="name_asc" {{ request('sort')=='name_asc' ? 'selected'
                                                        : '' }}>Name: A to Z</option>
                                                    <option value="name_desc" {{ request('sort')=='name_desc'
                    ...
                    <div class="active-filters mb-4">
                        ...
                        <div class="filter-tags">
                            ...
                            <span class="filter-tag">
                                ...
                                <a href="{{ route('shop', array_merge(request()->except('search'), ['page' => 1])) }}"
                            ...
                            <span class="filter-tag">
                                ...
                                return is_object($cat) ? $cat->id == request('category') : $cat['id'] ==
                                request('category');
                                });
                                $categoryName = $selectedCategory ? (is_object($selectedCategory) ?
                                $selectedCategory->name : $selectedCategory['name']) : 'Unknown';
                                @endphp
                                Category: {{ $categoryName }}
                                <a href="{{ route('shop', array_merge(request()->except('category'), ['page' => 1])) }}"
                                    class="remove-filter">×</a>
                            </span>
                            @endif
                            ...
                            <span class="filter-tag">
                                Price: ${{ request('min_price', $priceRange['min']) }} - ${{ request('max_price',
                                $priceRange['max']) }}
                                <a href="{{ route('shop', array_merge(request()->except(['min_price', 'max_price']), ['page' => 1])) }}"
                                    class="remove-filter">×</a>
                            </span>                            @endif
                ...
                <div class="row product-lists">
                    ...
                    <div class="col-lg-4 col-md-6 text-center mb-4">
                        <div class="single-product-item">
                            <div class="product-image">                                <a href="{{ route('product.show', $product->id) }}">
                            ...
                            <div class="product-info-section">
                                ...
                                <p class="product-price">
                                    ...
                                    <small>{{ implode(', ', $product->category_names->toArray() ?? ['Uncategorized'])
                                        }}</small>
                                </p>
                                ...
                                <div class="stock-status mb-2">
                                    @if($product->in_stock && $product->stock_qty > 0)
                                        <span class="badge badge-success">
                                            <i class="fas fa-check-circle"></i> In Stock ({{ $product->stock_qty }})
                                        </span>
                                    @else
                            ...
                            </div>                            <div class="product-action-buttons">
                                @if($product->in_stock && $product->stock_qty > 0)
                                    ...
                                    <button type="button" class="cart-btn disabled" disabled>
...
<style>
    ...
    .sidebar-focus {
        ...
        margin-left: -3px;
        /* Compensate for the border to avoid layout shift */
    }
    ...
    .price-symbol {
        ...
        line-height: normal;
        font-size: 15px;
        /* Match the font size with input */
        display: flex;
        align-items: center;
        height: 42px;
        /* Match the height with input */
    }
    ...
    @media only screen and (max-width: 768px) {
        body .single-product-item {
            min-height: 420px;
            /* Increased min-height for mobile to account for taller titles */
        }

        body .product-image {
            height: 180px;
            /* Slightly smaller image height for mobile */
        }
        ...
        body .single-product-item h3 {
            ...
            font-size: 14px;
            line-height: 1.4;
            /* Slightly tighter line-height for mobile */
        }
